...
Welle-39 LLM Sub-Funktion Extension: **HeyLou als Sub-Funktion in Mistral Function-Calling**.

Stack: Mistral Function-Calling API.
...
Mistral Function-Calling: Tool-Declaration als JSON-Schema, Modell ruft `functionCall` → unsere Extension routet zu HeyLou-API-Backend.
...
- `DF_HEYLOU_MISTRAL_EXT_ENABLED=false` → Mock-Responses (HeyLou Travel-Knowledge-Graph synthetisch)
- `DF_HEYLOU_MISTRAL_EXT_ENABLED=true` + `PHRONESIS_TICKET` + `MISTRAL_API_KEY` → Real-Mode

## Architektur

```
Mistral-LLM → functionCall → MistralExtension.handle_function_call()
