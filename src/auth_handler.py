"""Mistral-Auth-Handler fuer HeyLou-Extension [CRUX-MK].

Welle-39. User-OAuth (MISTRAL_API_KEY) + Service-Identity-Token (DF_HEYLOU_MISTRAL_HMAC_SECRET).
...
class MistralAuthHandler:
    """Gemini-Auth-Handler.

    Public-API:
    - get_credentials(tenant_id) -> MistralCredentials
    - validate(creds) -> bool
    ...
    def __init__(self, sandbox_mode: bool | None = None):
        if sandbox_mode is None:
            sandbox_mode = os.environ.get("DF_HEYLOU_MISTRAL_EXT_ENABLED", "false") != "true"
        self.sandbox_mode = sandbox_mode

    def _now_iso(self) -> str:
        return datetime.now(timezone.utc).isoformat()

    def get_credentials(self, tenant_id: str = "hildesheim") -> MistralCredentials:
        """Fetch credentials from ENV + tenant_id."""
        api_key = os.environ.get("MISTRAL_API_KEY", "")
        phronesis = (
            os.environ.get("PHRONESIS_TICKET")
            or os.environ.get("DF_HEYLOU_MISTRAL_PHRONESIS_TICKET")
        )

        if self.sandbox_mode:
            return MistralCredentials(
                api_key=api_key or "sandbox-fake-mistral-key",
    ...
    def validate(self, creds: MistralCredentials) -> bool:
