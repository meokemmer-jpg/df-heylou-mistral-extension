...
Phasen:
1. auth   -> MistralAuthHandler.get_credentials()
...
class MistralAdapterOrchestrator:
    DF_ID = DF_ID
    PROVIDER = PROVIDER

    def __init__(self, tenant_id: Optional[str] = None):
        self.tenant_id = tenant_id or os.environ.get(
            "DF_HEYLOU_MISTRAL_TENANT_ID", "hildesheim"
        )
        self.sandbox_mode = (
            os.environ.get("DF_HEYLOU_MISTRAL_EXT_ENABLED", "false") != "true"
        )
        ...
        self.extension = MistralExtension(sandbox_mode=self.sandbox_mode)
        self.auth = MistralAuthHandler(sandbox_mode=self.sandbox_mode)
...
if __name__ == "__main__":  # K16+K11-FOUNDATION-WIRED [CRUX-MK]
    ...
    raise SystemExit(_rg("df-heylou-mistral-extension", main))   # K14+K16+K15+K11 echt