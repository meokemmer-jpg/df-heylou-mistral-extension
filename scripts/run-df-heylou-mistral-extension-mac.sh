#!/bin/bash
# df-heylou-mistral-extension Mac LaunchAgent Runner [CRUX-MK]
# K16 concurrent_spawn protection via mkdir-Mutex
# Welle-39

set -euo pipefail

LOCK_DIR="/tmp/df-heylou-mistral-extension.lock"
...
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  echo "[df-heylou-mistral-extension] K16 lock held, exiting (ok)" >&2
  exit 3
fi
echo "$$" > "$LOCK_DIR/pid"
...
report = orch.run()
print(f'[df-heylou-mistral-extension] loop_id={report.loop_id} status={report.final_status} sandbox={report.sandbox_mode}')
