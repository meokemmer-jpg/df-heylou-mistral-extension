"""Tests for MistralExtension dispatch [CRUX-MK]."""
...
from src.mistral_extension import (
    ExtensionProvenance,
    ExtensionResponse,
    MistralExtension,
    K0_FUNCTIONS,
...
def test_book_direct_is_k0_relevant_in_sandbox_safe():
    ...
    assert resp.data["booking_id"].startswith("MOCK-mistral-")
...
def test_real_mode_requires_phronesis_ticket():
    ...
    assert ext.is_real_mode() is False
    os.environ.pop("DF_HEYLOU_MISTRAL_EXT_ENABLED", None)
...
def test_provenance_envelope_has_required_fields():
    ...
    assert p.extension_id == "df-heylou-mistral-extension"
