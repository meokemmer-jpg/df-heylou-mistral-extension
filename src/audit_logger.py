...
HMAC-SHA256-signed JSONL audit-entries (append-only, idempotent).
W30-G aligned. Schreibt zu audit/<target>-<DATE>.jsonl.
...
DEFAULT_DF_ID = "df-heylou-mistral-extension"
SECRET_ENV_VARS = (
    "DF_HEYLOU_MISTRAL_HMAC_SECRET",
    "DF_HEYLOU_EXT_HMAC_SECRET",
    "DF_SERVICE_IDENTITY_SECRET",
)
        ...
        if secret is None:
            for env_var in SECRET_ENV_VARS:
                ...
                if val:
                    ...
                    break
            if secret is None:
                secret = f"{DEFAULT_DF_ID}-runtime-default"
        return hmac.new(
...
class AuditLogger:
    ...
    DEFAULT_TARGETS = ["mistral-function-calls", "mistral-auth"]

    def __init__(self, audit_dir: str = "audit", df_id: str = DEFAULT_DF_ID):
    ...
    def log(
        ...
        event_type: str,
        payload: dict,
        target: str = "mistral-function-calls",
    ) -> AuditEntry:
        """Append signed audit-entry to JSONL.

        K11 try/except (failure-isolation). Returns the signed entry.
        """
        entry = AuditEntry(
        ...
        try:
            ...
            # Failure-isolation: nicht raisen
